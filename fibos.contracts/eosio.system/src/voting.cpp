...
void system_contract::regproducer(const account_name producer, const eosio::public_key& producer_key, const std::string& url, uint16_t location)
{
    ...
    if (prod != _producers.end()) {
        ...
        pststats pst_acnts(N(eosio.token), N(eosio.token));
        auto st = pst_acnts.find(producer);
        double _total_votals = 0;
        if (st != pst_acnts.end()) {
            _total_votals = st->amount.amount;
        }
        _producers.emplace(producer, [&](producer_info& info) {
            info.owner = producer;
            info.total_votes = _total_votals;
            info.producer_key = producer_key;
            info.is_active = true;
            info.url = url;
            info.location = location;
...
void system_contract::settotalvote(account_name owner, int64_t pst_amount)
{
    require_auth(N(eosio.token));
    auto prod = _producers.find(owner);

    if (prod != _producers.end()) {
        _producers.modify(prod, 0, [&](producer_info& info) {
            info.total_votes = pst_amount;
        });
    }
    /*else {
