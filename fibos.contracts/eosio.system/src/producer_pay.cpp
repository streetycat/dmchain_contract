...
void system_contract::onblock(block_timestamp timestamp, account_name producer)
{
    ...
    if (timestamp.slot - _gstate.last_producer_schedule_update.slot > 120) {
        ...
        INLINE_ACTION_SENDER(eosio::token, allocation)
        (N(eosio.token), { N(eosio), N(active) }, { "test" });

        if ((timestamp.slot - _gstate.last_name_close.slot) > blocks_per_day) {
            name_bid_table bids(_self, _self);
            auto idx = bids.get_index<N(highbid)>();
            auto highest = idx.lower_bound(std::numeric_limits<uint64_t>::max() / 2);
            if (highest != idx.end() && highest->high_bid > 0 && highest->last_bid_time < (current_time() - useconds_per_day) && _gstate.thresh_activated_stake_time > 0 && (current_time() - _gstate.thresh_activated_stake_time) > 14 * useconds_per_day) {
...
void system_contract::claimrewards(const account_name& owner)
{
    ...
    if (producer_per_block_pay > 0) {
        INLINE_ACTION_SENDER(eosio::token, transfer)
        (N(eosio.token), { { N(eosio.bpay), N(active) }, { owner, N(active) } },
            { N(eosio.bpay), owner, asset(producer_per_block_pay), std::string("producer block pay") });
    }
    if (producer_per_vote_pay > 0) {
        INLINE_ACTION_SENDER(eosio::token, transfer)
        (N(eosio.token), { { N(eosio.vpay), N(active) }, { owner, N(active) } },
