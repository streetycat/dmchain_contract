...
void system_contract::buyram(account_name payer, account_name receiver, asset quant)
{
    ...
    (N(eosio.token), { { payer, N(active) }, { N(eosio.ram), N(active) } },
...
void system_contract::sellram(account_name account, int64_t bytes)
{
    ...
    eosio_assert(res_itr->ram_bytes >= 4096, "the remaining ram must be greater than 4096");
    set_resource_limits(res_itr->owner, res_itr->ram_bytes + ram_gift_bytes, res_itr->net_weight.amount, res_itr->cpu_weight.amount);

    INLINE_ACTION_SENDER(eosio::token, transfer)
    (N(eosio.token), { { N(eosio.ram), N(active) }, { account, N(active) } },
    ...
    if (fee > 0) {
        INLINE_ACTION_SENDER(eosio::token, transfer)
        (N(eosio.token), { account, N(active) },
    ...
    const asset stake_net_delta, const asset stake_cpu_delta, bool transfer)
{
    ...
    if (N(eosio.stake) != source_stake_from) { // for eosio both transfer and refund make no sense
        ...
        if (transfer_amount > asset(0)) {
            INLINE_ACTION_SENDER(eosio::token, transfer)
            (N(eosio.token), { source_stake_from, N(active) },
...
void system_contract::refund(const account_name owner)
{
    ...
    INLINE_ACTION_SENDER(eosio::token, transfer)
    (N(eosio.token), { { N(eosio.stake), N(active) }, { req->owner, N(active) } },
