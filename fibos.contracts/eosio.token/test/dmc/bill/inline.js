...
describe(`bill`, () => {
    ...
    it(`success, bill 100 PST`, () => {
        ...
        console.warn('---- fibos.getTableRowsSync(true, "eosio.token", "fibos", "stakerec").rows ----', fibos.getTableRowsSync(true, "eosio.token", "fibos", "stakerec").rows);
    ...
    it(`multiple bill`, () => {
        ...
        for (var i = 0; i <= 3; i++) {
            let amount = 50 + i;
            coroutine.sleep(1000);
            ctx.billSync("fibos", `${amount} PST@datamall`, 0, `test ${amount}`, {
                authorization: "fibos"
            });
        }
        console.warn('---- fibos.getTableRowsSync(true, "eosio.token", "fibos", "stakerec").rows ----', fibos.getTableRowsSync(true, "eosio.token", "fibos", "stakerec").rows);
