...
describe(`price avg`, () => {
    ...
    it(`check price avg`, () => {
        ...
        assert.deepEqual(p, [{
            "primary": 0,
            "price": "2.00000000000000000",
        }, {
            "primary": 1,
            "price": "2.00000000000000000",
        }, {
            "primary": 2,
            "price": "2.09999999986030161",
        }, {
            "primary": 3,
            "price": "2.09999999986030161",
        }])
        let avg = fibos.getTableRowsSync(true, "eosio.token", "eosio.token", "priceavg").rows;
        assert.deepEqual(avg, [
            {
                "primary": 0,
                "total": "8.19999999972060323",
                "count": 4,
                "avg": "2.04999999993015081"
            }
        ])
    })

    // only check with 
    // @code constexpr uint64_t price_fluncuation_interval = 5;
    xit(`check erase`, () => {
        coroutine.sleep(5);
        let p = fibos.getTableRowsSync(true, "eosio.token", "eosio.token", "dmcprice").rows;
        console.warn('---- p ----', p);
